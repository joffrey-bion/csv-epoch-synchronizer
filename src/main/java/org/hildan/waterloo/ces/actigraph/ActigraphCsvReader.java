...
/**
 * A reader for the CSV file produced by ActiLife when exporting the actigraph's epochs. All the
 * constants relative to the organization of the CSV file are hardcoded in this class.
 *
...
public class ActigraphCsvReader extends TimestampedCsvReader {
     ...
     */
    public ActigraphCsvReader(String filename, ActigraphFileFormat actigraphFileFormat) throws FileNotFoundException,
    ...
    public long extractTimestamp(String[] line) throws IOException {
        final String timestamp = line[f.DATE_COL] + " " + line[f.TIME_COL];
        final String format = f.DATE_FORMAT + " " + f.TIME_FORMAT;
        try {
    ...
    public double extractCountsPerMinutes(String[] line, long epochWidthNanos) {
        final long nbEpochsPerMin = (long) 60 * 1000 * 1000000 / epochWidthNanos;
