...
/**
 * A module that appends the level decided by a decision tree to each row of a CSV file according to
 * the values for each feature (column).
 *
...
public class LabelAppender {
    ...
    private final HashMap<String, Integer> lvlsDistrib;
    ...
    /**
     * Determines the level of each row of the dataset file according to the decision tree, and
     * appends it as a new column to the dataset in the destination file.
     *
     ...
     */
    public static HashMap<String, Integer> appendLabels(String classifierFile, String datasetFile, String outputPath,
            boolean removeTimestamps) throws NotACsvFileException, SAXException, IOException {
        System.out.println("Opening dataset file '" + datasetFile + "'...");
        final CsvReader reader = new CsvReader(datasetFile);
        String[] row = reader.readRow();
        final LabelAppender la = new LabelAppender(classifierFile);
        la.indexFeatures(row);
        System.out.println("Classifying...");
        final CsvWriter writer = new CsvWriter(outputPath);
     ...
     *
     * @param params
     *            The {@link LabelAppenderParams} object containing all the parameters to use.
     * @return The levels distribution of the dataset.
     * @throws IOException
     *             If any IO error occurs.
     * @throws SAXException
     *             If any parse error occurs when reading the classifier XML file.
     */
    public static HashMap<String, Integer> appendLabels(LabelAppenderParams params) throws IOException, SAXException {
     ...
     *
     * @param headers
     *            The header line of the CSV dataset containing the names of the features.
     */
    ...
    /**
     * Determines the level of the specified row and appends it at the end of this row.
     *
     * @param row
    ...
    private String[] appendLabel(String[] row, boolean removeTimestamp) {
        final String label = tree.classify(row, features);
        final int oldNb = lvlsDistrib.containsKey(label) ? lvlsDistrib.get(label) : 0;
     ...
     *            Whether the first column should be removed (shift left when appending).
    ...
    private static String[] append(String[] tab, String element, boolean removeFirstCol) {
        ...
        final String[] res = Arrays.copyOfRange(tab, startIndex, tab.length + 1);
