...
/**
 * A {@code DecisionTree} object can be viewed as a subtree of a decision tree. It is either an
 * internal node or a leaf.
 * <p>
 * If it is a leaf, then it represents a class. If it is an internal node, then it represents a
 * feature that has to be compared to a threshold.
 * </p>
 * <p>
 * On a given sample, if the specified feature of the sample is lower than or equal to the
 * threshold, then we move to the left (low) son. Otherwise, we move to the right (high) son.
 * Anyway, we carry on until a leaf is reached, giving the class of the tested sample.
 * </p>
 *
...
class DecisionTree {
     ...
     * @param threshold
     *            The threshold to choose between lowSon or highSon. If the feature is less than or
     *            equal to the threshold, then the lowSon has to be considered, otherwise the
     *            highSon branch is followed.
     * @param lowSon
     ...
     * @param features
     *            A map between the name of each feature and its index in the {@code attributes}
     *            array.
    ...
    public String classify(String[] attributes, HashMap<String, Integer> features) {
        ...
        final Integer featureIndex = features.get(feature);
     ...
     *
     * @param indent
     *            The indentation to append before each line. May contain vertical lines, or
     *            branches. Middle branches will be turned into vertical lines (to continue the
     *            other branches), end branches will be replaced by blanks (because the branch is
     *            finished).
