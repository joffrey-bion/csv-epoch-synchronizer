...
import org.hildan.waterloo.ces.k4b2.METsToLevel;
...
public class PhaseResults extends StatsWindow {
    ...
    private final HashMap<String, Double> lvlDistrib;

    private final METsToLevel mtl;
    ...
    public void add(Sample line) {
        super.add(line);
        final double METs = line.getValue(Sample.COL_METS);
        final String level = mtl.metsToLevel(METs);
        final double oldWeight = lvlDistrib.containsKey(level) ? lvlDistrib.get(level) : 0;
        lvlDistrib.put(level, oldWeight + line.duration / 1000);
    }

    @Override
    public boolean remove(Sample line) {
        final boolean removed = super.remove(line);
        if (removed) {
            final double METs = line.getValue(Sample.COL_METS);
            final String level = mtl.metsToLevel(METs);
            final double oldWeight = lvlDistrib.containsKey(level) ? lvlDistrib.get(level) : 0;
     ...
     *
     * @return A map between the level names and the associated cumulated duration in seconds.
     */
    ...
    /**
     * Returns the average METs for this phase. This method uses the value of the resting VO2/kg set
     * via {@link #setRestingVO2kg(double)} to compute individual METs values.
     *
    ...
    public String toString() {
        ...
        String res = DurationHelper.toTime(getStartTime()) + " to " + DurationHelper.toTime(getEndTime()) + " ("
