...
/**
 * A {@code StatsWindow} sets a duration limit for a group of values. When adding new values, it
 * fills the time window until the maximum duration is reached, and then returns the oldest value
 * for each new value inserted.
 *
...
class StatsWindow {
    ...
    private final K4b2SampleStats stats;
    ...
    public void add(Sample line) {
        final LinkedList<Sample> linesToAdd = new LinkedList<>();
        linesToAdd.add(line);
        add(linesToAdd);
    }
     ...
     *            list will be cleared and receive the oldest samples in this group that do not fit
     *            anymore in the maximum length. The oldest of these samples is added first in the
     *            list, the newest is added last.
     */
    public void add(LinkedList<Sample> linesToAdd) {
        while (!linesToAdd.isEmpty()) {
            final Sample sample = linesToAdd.pollFirst();
            stats.add(sample);
            samples.add(sample);
    ...
    public boolean remove(Sample line) {
        ...
        final boolean removed = samples.remove(line);
        if (removed) {
            stats.remove(line);
    ...
    public Sample removeOldest() {
        final Sample oldest = samples.getFirst();
        remove(oldest);
        return oldest;
    }
    ...
    public Sample removeNewest() {
        final Sample newest = samples.getLast();
        remove(newest);
        return newest;
    }
    ...
    /**
     * Returns the current duration of this window in milliseconds. This corresponds to the actual
     * sum of the durations of all the samples contained in this window, not to the desired
     * duration.
     *
    ...
    /**
     * Removes the oldest (first) samples from this window. This window's duration is shortened to
     * reach {@code target} milliseconds.
     *
     * @param targetDuration
     *            The target duration for this window.
     */
    public void trimBeginningToReach(long targetDuration) {
        final long target = targetDuration < 0 ? 0 : targetDuration;
    ...
    /**
     * Removes the newest (last) samples from this window. This window's duration is shortened to
     * reach {@code targetDuration} milliseconds.
     *
     * @param targetDuration
     *            The target duration for this window.
     */
    ...
    /**
     * Removes the oldest (first) samples from this window. This window's duration is shortened by
     * {@code millisToStop} milliseconds.
     *
     * @param millisToSkip
    ...
    /**
     * Removes the newest (last) samples from this window. This window's duration is shortened by
     * {@code millisToStop} milliseconds.
     *
     * @param millisToSkip
     *            The amount of time that has to be cut from this window.
