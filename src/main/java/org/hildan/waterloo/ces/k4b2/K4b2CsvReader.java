...
class K4b2CsvReader extends CsvReader {
    ...
    public Sample readK4b2Sample() throws IOException {
        ...
        try {
            final long previousTime = lastReadSample != null ? lastReadSample.endTime : 0;
    ...
    /**
     * Skips the next {@code nbToSkip} markers (including the current one if the current row is
     * marked) and goes to the next marked row.
     *
     * @param nbToSkip
    ...
    /**
     * Reads as many rows as necessary to reach the next row with a marker. If the current row is
     * marked, it is not taken into account and the next row with a marker is returned.
     *
    ...
    public Sample goToNextMarker() throws IOException {
        ...
        throw new EOFException("The end of file has been reached while looking for the next marker (" + count
                + " rows read).");
    }
