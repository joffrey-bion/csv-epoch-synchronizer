...
public class RestingResults extends PhaseResults {
    ...
    private final LinkedList<StatsWindowsGroup> windows;
    ...
    public void add(Sample sample) {
        super.add(sample);
        final LinkedList<Sample> toMove = new LinkedList<>();
        toMove.add(sample);
        long cumulatedDuration = 0;
        int nbWindows = 0;
        // fill first window, and move extra old lines to next window
        for (final StatsWindowsGroup win : windows) {
        ...
        while (!toMove.isEmpty()) {
            final StatsWindowsGroup win = new StatsWindowsGroup(GROUP_LENGTH_MILLIS, SUBGROUP_LENGTH_MILLIS);
            windows.add(win);
            win.setCompensation(cumulatedDuration, nbWindows);
            win.add(toMove);
            cumulatedDuration += win.getDuration();
    ...
    /**
     * Returns a readable {@code String} describing the values in each 5min period during the
     * resting phase.
     *
    ...
    public String allToString() {
        ...
        final Iterator<StatsWindowsGroup> it = windows.descendingIterator();
    ...
    /**
     * Returns a readable {@code String} describing the values in the last 5min of the resting
     * phase.
     *
