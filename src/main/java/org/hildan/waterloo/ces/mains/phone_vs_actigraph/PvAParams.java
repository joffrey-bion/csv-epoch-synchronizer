...
import org.hildan.utils.csv.Csv;
...
import org.hildan.utils.xml.serialization.serializers.DateArraySerializer;
import org.hildan.utils.xml.serialization.serializers.DateSerializer;
...
ClassificationAnalysisParams {
    ...
    static {
        SCHEMA.addParam(OUTPUT_TRAINING_SET_FILE, SimpleSerializer.STRING, "Name of the output dataset file");
        SCHEMA.addParam(OUTPUT_VALIDATION_FILE, SimpleSerializer.STRING, "Name of the output validation results file");
        ...
        SCHEMA.addOptionalParam(ACTIG_FILE_FORMAT, FORMAT_SER, DEFAULT_ACTIG_FILE_FORMAT,
                "EXPORTED (via \"export all epochs\") or CONVERTED (via File > Import/Export/Convert)");
        SCHEMA.addOptionalParam(EPOCH_WIDTH_SEC, SimpleSerializer.INTEGER, DEFAULT_EPOCH_WIDTH_SEC,
                "The epoch width of the actigraph file in seconds");
        SCHEMA.addParam(START_TIME, TIMESTAMP_SER, "Start time in actigraph reference (" + TIMESTAMP_FORMAT + ")");
        SCHEMA.addParam(STOP_TIME, TIMESTAMP_SER, "Stop time in actigraph reference (" + TIMESTAMP_FORMAT + ")");
        SCHEMA.addOptionalParam(PROFILE, PROFILE_SER, Profile.POCKET);
        SCHEMA.addOptionalParam(PHONE_LOCATION, PHONE_LOCATION_SER, PhoneLocation.LEFT);
        SCHEMA.addOptionalParam(PHONE_TYPE, PHONE_TYPE_SER, PhoneType.GYRO);
        SCHEMA.addParam(PHONE_SPIKES_LIST, SPIKES_SER, "Phone acceleration spikes, in the right order ("
                + TIMESTAMP_FORMAT + ")");
        SCHEMA.addParam(ACTIG_SPIKES_LIST, SPIKES_SER, "Actigraph acceleration spikes, in the right order ("
                + TIMESTAMP_FORMAT + ")");
    }
    ...
    /**
     * Delay in nanoseconds to add to a phone time to find the corresponding actigraph time.
     */
    private long delay;
     ...
     */
    public PvAParams(String paramsFilePath) throws IOException, SAXException, SpecificationNotMetException {
        super(SCHEMA);
    ...
    /**
     * Creates a new empty {@link PvAParams} object that has to be filled via {@link Parameters}'
     * methods. When all parameters are set, the method {@link #populatePublicFields()} has to be
     * called to make the public fields of this class usable.
     */
    public PvAParams() {
        super(SCHEMA);
    }
    ...
    /**
     * Pull the data from this {@link Parameters} object and make it available more efficiently
     * through the public fields and getters.
     */
    public void populatePublicFields() {
        ...
        final Long[] phoneSpikes = get(PHONE_SPIKES_LIST, SPIKES_SER);
        final Long[] actigSpikes = get(ACTIG_SPIKES_LIST, SPIKES_SER);
        setDelay(phoneSpikes, actigSpikes);
    }

    @Override
    public void loadFromXml(String xmlFilePath) throws IOException, SAXException, SpecificationNotMetException {
    ...
    /**
     * Replace an absolute path parameter by a path relative to the specified {@code basePath}.
     *
     * @param key
    ...
    /**
     * Calculate the delay between the phone's time and the actigraph's time according to the given
     * spikes timestamps in both references. These spikes were obtained by shaking both devices
     * together.
     *
     * @param phoneSpikes
     *            Timestamps of the spikes in the phone's time reference, in milliseconds.
     * @param actigraphSpikes
     *            Timestamps of the same spikes in the actigraph's reference, in milliseconds.
     */
    private void setDelay(Long[] phoneSpikes, Long[] actigraphSpikes) {
        final FlowStats delayStats = new FlowStats();
        long phoneTime;
        long actigraphTime;
