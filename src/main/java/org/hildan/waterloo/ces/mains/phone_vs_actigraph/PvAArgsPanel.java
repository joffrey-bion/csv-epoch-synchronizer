...
class PvAArgsPanel extends JPanel {
    ...
    private final JTextField tfStopTime;

    private final JLabel[] tfSpikeLabel;

    private final JTextField[] tfSpikePhone;

    private final JTextField[] tfSpikeActig;

    private final JTextField tfEpochWidth;

    private final JComboBox<ActigraphFileFormat> cBoxActigraphFileFormat;
    ...
    public PvAArgsPanel(JFilePickersPanel filePickers) {
        ...
        final JPanel panelArgsLeft = new JPanel();
        ...
        final JPanel panelLimits = new JPanel();
        panelLimits.setAlignmentX(Component.LEFT_ALIGNMENT);
        panelArgsLeft.add(panelLimits);
        final GridBagLayout gbl_panelLimits = new GridBagLayout();
        ...
        final JLabel lblNewLabel = new JLabel("Processing Limits (actigraph time)");
        ...
        final GridBagConstraints gbc_lblStartTime = new GridBagConstraints();
        ...
        final GridBagConstraints gbc_tfStartTime = new GridBagConstraints();
        ...
        final JLabel lblStopTime = new JLabel("Stop time:");
        final GridBagConstraints gbc_lblStopTime = new GridBagConstraints();
        ...
        final GridBagConstraints gbc_tfStopTime = new GridBagConstraints();
        ...
        final JLabel lblFormat = new JLabel("(" + PvAParams.TIMESTAMP_FORMAT + ")");
        final GridBagConstraints gbc_lblFormat = new GridBagConstraints();
        ...
        final JLabel lblEpochWidth = new JLabel("Actigraph epochs length (sec):");
        final GridBagConstraints gbc_lblEpochWidth = new GridBagConstraints();
        ...
        final GridBagConstraints gbc_tfEpochWidth = new GridBagConstraints();
        ...
        final JLabel lblActigraphEpochFile = new JLabel("Actigraph file format:");
        final GridBagConstraints gbc_lblActigraphEpochFile = new GridBagConstraints();
        ...
        final GridBagConstraints gbc_cBoxActigraphFileFormat = new GridBagConstraints();
        ...
        final JLabel lblSpikesTitle = new JLabel("Spikes (" + PvAParams.TIMESTAMP_FORMAT + ")");
        ...
        final JLabel lblPhone = new JLabel("Phone time");
        ...
        final JLabel lblActigraph = new JLabel("Actigraph time");
        final GridBagConstraints gbc_lblActigraph = new GridBagConstraints();
        ...
        for (int i = 0; i < NB_MAX_SPIKES; i++) {
            ...
            final GridBagConstraints gbc_tfSpikeActig = new GridBagConstraints();
        ...
        final SaveLoadPanel psp = new SaveLoadPanel("Save params...", "Load params...") {
            ...
            public void loadFromFile(String paramFilePath) {
                try {
                    final PvAParams params = new PvAParams(paramFilePath);
    ...
    private void setParameters(PvAParams params) throws MissingParameterException {
        ...
        inputs[PhoneVSActigraph.INPUT_ACTIGRAPH]
                .setSelectedFilePath(params.getString(PvAParams.INPUT_ACTIG_EPOCH_FILE));
        inputs[PhoneVSActigraph.INPUT_PARTICIPANT].setSelectedFilePath(params
                .getString(PvAParams.INPUT_PARTICIPANT_FILE));
        final FilePicker[] outputs = filePickers.getOutputFilePickers();
        outputs[PhoneVSActigraph.OUTPUT_TRAINING_SET].setSelectedFilePath(params
                .getString(PvAParams.OUTPUT_TRAINING_SET_FILE));
        outputs[PhoneVSActigraph.OUTPUT_VALIDATION].setSelectedFilePath(params
                .getString(PvAParams.OUTPUT_VALIDATION_FILE));
        ...
        final String[] phoneSpikes = params.getSerializedArray(PvAParams.PHONE_SPIKES_LIST);
        for (int i = 0; i < phoneSpikes.length; i++) {
            tfSpikePhone[i].setText(phoneSpikes[i]);
        }
        final String[] actigraphSpikes = params.getSerializedArray(PvAParams.ACTIG_SPIKES_LIST);
    ...
    /**
     * Fills the specified {@link PvAParams} object with the current fields of this panel (including
     * the files paths).
     *
     * @param params
    ...
    public void getParameters(PvAParams params) throws ParseException {
        ...
        setFileParam(params, PvAParams.INPUT_PARTICIPANT_FILE,
                filePickers.getInputFilePaths()[PhoneVSActigraph.INPUT_PARTICIPANT]);
        setFileParam(params, PvAParams.INPUT_ACTIG_EPOCH_FILE,
                filePickers.getInputFilePaths()[PhoneVSActigraph.INPUT_ACTIGRAPH]);
        setFileParam(params, PvAParams.OUTPUT_VALIDATION_FILE,
                filePickers.getOutputFilePaths()[PhoneVSActigraph.OUTPUT_VALIDATION]);
        setFileParam(params, PvAParams.OUTPUT_TRAINING_SET_FILE,
                filePickers.getOutputFilePaths()[PhoneVSActigraph.OUTPUT_TRAINING_SET]);
        ...
        if (nbSpikes > 0) {
            params.deserializeAndSet(PvAParams.PHONE_SPIKES_LIST, Arrays.copyOfRange(phoneSpikes, 0, nbSpikes));
            params.deserializeAndSet(PvAParams.ACTIG_SPIKES_LIST, Arrays.copyOfRange(actigraphSpikes, 0, nbSpikes));
        } else {
    ...
    private static void setIfNotEmpty(PvAParams params, String key, String value) throws ParseException {
        if (!"".equals(value)) {
