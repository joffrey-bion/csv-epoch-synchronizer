...
public class Main {
    ...
    private static final int ARG_PHONE_RAW_FILE = 1;
    private static final int ARG_ACTIGRAPH_EPOCH_FILE = 2;
    private static final int ARG_OUTPUT_FILE = 3;
    private static final int ARG_START_TIME = 4;
    private static final int ARG_STOP_TIME = 5;

    private static final int NB_ARGS_BEFORE_SPIKES = 6;
    ...
    private static void openWindow() {
        ...
        JFilePickersPanel filePickers = new JFilePickersPanel(new String[] { "Phone raw file",
                "Actigraph epoch file" }, "Output file");
        @SuppressWarnings("serial")
        JFileProcessorWindow frame = new JFileProcessorWindow("Epoch Synchronizer",
                "Process", filePickers, argsPanel) {
            @Override
    ...
    private static void createDataset(InstanceProperties props, Logger log) {
        ...
        try {
            ...
            log.println("");
            log.println("Actigraph start time: " + DateHelper.toDateTimeMillis(props.startTime / 1000000));
            ...
            if (new File(phoneEpFilename).delete()) {
                log.println("> Intermediate file deleted");
            } else {
                log.println("> Delete operation has failed.");
            }
        } catch (IOException e) {
            log.printErr(e.getMessage());
        }
        log.println("");
        log.println("Total processing time: " + (System.currentTimeMillis() - timerStart)
                + "ms");
        log.println("Output file: " + props.outputFilename);
    }
